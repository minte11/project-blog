...
function DivisionGroupsDemo ({numOfItems = 12, initialNumOfGroups = 1, includeRemainderArea,}) {
	...
	return (<Card as="section" className={styles.wrapper}>
		<LayoutGroup>
			...
			{includeRemainderArea && (<div className={styles.remainderArea}>
				...
				{range(remainder).map((index) => {
					...
					const layoutId = `${id}-${dotId}`;
					return (<motion.div key={layoutId} layoutId={layoutId} className={styles.item}>{dotId}</motion.div>);
				})}
			</div>)}
